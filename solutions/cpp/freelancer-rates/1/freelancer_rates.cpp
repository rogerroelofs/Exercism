...
int monthly_rate(double hourly_rate, double discount) {
    return static_cast<int>(std::ceil(discounted_daily_rate(hourly_rate, discount) * 22));
}
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    return static_cast<int>(std::floor(budget / discounted_daily_rate(hourly_rate, discount)));
}